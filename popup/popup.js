...
function restoreFilterState() {
  chrome.storage.session.get(['filterCategory', 'sortOrder', 'favoritesOnly', 'searchQuery'], (result) => {
    if (result.filterCategory) filterCategory.value = result.filterCategory;
    if (result.sortOrder) sortOrder.value = result.sortOrder;
    if (result.favoritesOnly === true) btnFavorites.setAttribute('aria-pressed', 'true');
...
function renderList(prompts) {
  ...
  prompts.forEach((p) => {
    ...
    const bodyPreview = p.body.length > 120 ? p.body.slice(0, 120) + '…' : p.body;
...
function injectPrompt(prompt) {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.tabs.sendMessage(tab.id, { type: 'INJECT_PROMPT', text: prompt.body }, (response) => {
...
function importPrompts(file) {
  ...
  reader.onload = (e) => {
    ...
    db.getAllPrompts().then((existing) => {
      const existingTitles = new Set(existing.map((p) => p.title.toLowerCase()));

      const importPromises = [];
      records.forEach((record) => {
        if (!record.title || !record.body || typeof record.title !== 'string' || typeof record.body !== 'string') {
          skippedCount++;
          return;
        }
      ...
      refreshList();
      showToast('Imported ' + importedCount + ', skipped ' + skippedCount, 'success');
    }).catch((err) => {
